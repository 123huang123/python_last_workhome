...
for i in range(len(blogList)):
    ...
    if ("秒" in str(childListOfFromElement[0].text)):
        time=str(datetime.now().strftime("%Y-%m-%d %H:%M"))
    if ("分" in str(childListOfFromElement[0].text)):
        time1 = int(re.findall(r'\d+(?:\.\d+)?', str(childListOfFromElement[0].text))[0])
        time=str((datetime.now()+timedelta(minutes=-time1)).strftime("%Y-%m-%d %H:%M"))
